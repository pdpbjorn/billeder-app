...
function initAdaptiveNav() {
  ...
  $(document)
    ...
    .off("click", "#nav-open-panel")
    .on("click", "#nav-open-panel", function () {
    ...
    .off("click", "#navApplySelection")
    .on("click", "#navApplySelection", applyPendingSelection)
...
function openNavDrawer() {
  ...
  window.setTimeout(() => {
    $("#navDrawer .drawer-option:visible, #navDrawer .drawer-action:visible").first().trigger("focus");
  }, 30);
}
  ...
  if (root === "tid") {
    ...
    if (navState.stack.length === 0) {
      return tid.decades.map(dec => ({
        ...
        hasChildren: true,
        onSelect: () => { navState.stack.push({ label: dec.id, node: dec }); renderNavDrawer(); }
      }));
    }
    if (navState.stack.length === 1) {
      ...
      return (dec.years || []).map(yr => ({
        ...
        hasChildren: true,
        onSelect: () => { navState.stack.push({ label: yr.id, node: yr }); renderNavDrawer(); }
      }));
    }
  ...
  if (root === "anvendelse") {
    ...
    if (navState.stack.length === 0) {
      return (anv.groups || []).map(group => ({
        ...
        hasChildren: true,
        onSelect: () => { navState.stack.push({ label: group.group, node: group }); renderNavDrawer(); }
      }));
    }
...
function renderNavDrawer() {
  ...
  $("#navBreadcrumb").text(getBreadcrumbText());
  $("#navDrawerBack").text(navState.stack.length > 0 ? "Tilbage" : "Luk");

  const $body = $("#navDrawerBody");
  $body.empty();

  if (navState.currentRoot === "tid" && navState.stack.length === 0) {
    const noDateSelected = navState.pendingSelection?.key === "tid:no-date";
    $("<button/>", {
      type: "button",
      class: "drawer-secondary-option" + (noDateSelected ? " is-selected" : ""),
  ...
  items.forEach(item => {
    const isSelected = navState.pendingSelection?.key && item.key === navState.pendingSelection.key;
    const $btn = $("<button/>", {
      type: "button",
    ...
    $("<span/>", { class: "drawer-chevron", html: item.hasChildren ? "›" : "" }).appendTo($btn);
    $btn.on("click", item.onSelect);
    $body.append($btn);
  });

  $("#navApplySelection").prop("disabled", !navState.pendingSelection);
}

function applyPendingSelection() {
  if (!navState.pendingSelection) return;
  navState.activeSelection = navState.pendingSelection;
  updateSelectionLabel();
  closeNavDrawer();
  navState.pendingSelection.action();
}
...
function handleDrawerKeyNav(e) {
  const $focusables = $("#navDrawer .drawer-option:visible, #navDrawer .drawer-action:visible, #navDrawer .drawer-footer-btn:visible");
