...
function resolveKmlIconUrl(url) {
  ...
  // Google Earth's old "flatfile" icon URLs look like:
  //   http://kh.google.com:80/flatfile?lf-0-icons/information_new_n.png
  // The *real* filename is in the query string, not the pathname.
  const q = (u.split("?")[1] || "").trim();
  if (q) {
    const firstPart = q.split("&")[0]; // defensive: ignore any additional params
    const qFilename = firstPart.substring(firstPart.lastIndexOf("/") + 1);
    if (qFilename && qFilename.toLowerCase().endsWith(".png")) {
      return `icons/KML/${qFilename}`;
    }
  }

  // Normal remote URL: use the last path segment
  const pathOnly = u.split("?")[0];
  const filename = pathOnly.substring(pathOnly.lastIndexOf("/") + 1);

  // If we ended up with "flatfile" (no extension), still try query-derived name
  if (!filename || filename.toLowerCase() === "flatfile") {
    if (q) {
      const firstPart = q.split("&")[0];
      const qFilename = firstPart.substring(firstPart.lastIndexOf("/") + 1);
      if (qFilename) return `icons/KML/${qFilename}`;
    }
  }
