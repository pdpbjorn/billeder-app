...
function syncViewToggleState() {
  const root = navState.currentRoot || "tid";

  if (!navState.viewByRoot[root]) {
    navState.viewByRoot[root] = root === "tid" ? "list" : "map";
  }

  if (document.getElementById("Choice-of-list")?.checked) {
    navState.viewByRoot[root] = "list";
  }

  if (document.getElementById("Choice-of-map")?.checked) {
    navState.viewByRoot[root] = "map";
  }
...
function syncViewToggleUI() {
  const root = navState.currentRoot || "tid";
  const mode = navState.viewByRoot[root] || (root === "tid" ? "list" : "map");

  $("#Choice-of-list").prop("checked", mode === "list");
  $("#Choice-of-map").prop("checked", mode === "map");
...
function isListMode(){
  const root = navState.currentRoot || "tid";
  return (navState.viewByRoot[root] || (root === "tid" ? "list" : "map")) === "list";
}
