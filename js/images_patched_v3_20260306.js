...
const navState = {
  ...
  timeDesktop: { decadeId: null, yearId: null }
};
...
function openNavRoot(root) {
  ...
  if (root === "tid") {
    navState.timeDesktop = { decadeId: null, yearId: null };
  }
...
function getBreadcrumbText() {
  if (isDesktopTimeChooser()) {
    const parts = [];
    if (navState.timeDesktop.decadeId) parts.push(navState.timeDesktop.decadeId);
    if (navState.timeDesktop.yearId) parts.push(navState.timeDesktop.yearId);
    return parts.join(" › ");
  }
...
function getCurrentTimeDesktopDecade() {
  const decades = appNavData.tid?.decades || [];
  let decade = decades.find(d => d.id === navState.timeDesktop.decadeId);
  if (!decade && decades.length) {
    decade = decades[0];
    navState.timeDesktop.decadeId = decade.id;
  }
  return decade || null;
}

function getCurrentTimeDesktopYear() {
  const decade = getCurrentTimeDesktopDecade();
  const years = decade?.years || [];
  let year = years.find(y => y.id === navState.timeDesktop.yearId);
  if (!year && years.length) {
    year = years[0];
    navState.timeDesktop.yearId = year.id;
  }
  return year || null;
}
...
function getDrawerHint() {
  if (navState.currentRoot === "tid") {
    if (isDesktopTimeChooser()) return "Vælg årti, år og måned i de tre kolonner. Måned åbner straks i " + (isListMode() ? "liste" : "kort") + "-visning.";
...
function renderTimeDesktopChooser($body) {
  const tid = appNavData.tid || { decades: [] };
  const decades = tid.decades || [];
  const decade = getCurrentTimeDesktopDecade();
  const years = decade?.years || [];
  const year = getCurrentTimeDesktopYear();
  const months = year?.months || [];

  const makeColumn = (title, items, activeId, onPick, metaGetter) => {
    const $col = $("<section/>", { class: "time-column" });
    $("<div/>", { class: "time-column-title", text: title }).appendTo($col);
    const $list = $("<div/>", { class: "time-column-list" }).appendTo($col);
    if (!items.length) {
      $("<div/>", { class: "time-column-empty", text: "Ingen valg" }).appendTo($list);
      return $col;
    }
    items.forEach(item => {
      ...
      const $btn = $("<button/>", {
        type: "button",
        class: "drawer-option time-column-option" + (isActive ? " is-active" : "")
      });
      const $main = $("<span/>", { class: "drawer-option-main" }).appendTo($btn);
      $("<span/>", { class: "drawer-option-label", text: item.id }).appendTo($main);
      const meta = metaGetter ? metaGetter(item) : "";
      if (meta) $("<span/>", { class: "drawer-option-meta", text: meta }).appendTo($main);
      $("<span/>", { class: "drawer-chevron", html: title === "Måneder" ? "Åbn" : "›" }).appendTo($btn);
      $btn.on("click", () => onPick(item));
      $list.append($btn);
    });
    return $col;
  };

  $body.append(makeColumn("Årtier", decades, decade?.id, (item) => {
    navState.timeDesktop.decadeId = item.id;
    navState.timeDesktop.yearId = item.years?.[0]?.id || null;
    renderNavDrawer();
  }, item => item.years?.length ? item.years.length + " år" : ""));

  $body.append(makeColumn("År", years, year?.id, (item) => {
    navState.timeDesktop.yearId = item.id;
    renderNavDrawer();
  }, item => item.months?.length ? item.months.length + " måneder" : ""));

  $body.append(makeColumn("Måneder", months, null, (item) => {
    selectAndApply({
      key: "tid:" + item.id,
      label: "Tid: " + (decade?.id || "") + " › " + (year?.id || "") + " › " + item.id,
      action: () => injectMonth(item.id)
    });
  }, item => typeof item.count === "number" ? item.count + " billeder" : "Måned"));
}

function renderNavDrawer() {
  $("#navDrawerTitle").text(getDrawerTitle());
  $("#navBreadcrumb").text(getBreadcrumbText()).toggleClass("is-hidden", !getBreadcrumbText());
  ...
  if (isDesktopTimeChooser()) {
    ...
    const $secondaryWrap = $("<div/>", { class: "time-secondary-row" });
    $("<button/>", {
      type: "button",
    ...
    }).appendTo($secondaryWrap);
    $body.append($secondaryWrap);
    return;
  }
...
function handleDrawerKeyNav(e) {
  ...
  if (e.key === "ArrowDown" || e.key === "ArrowRight") {
    ...
    e.preventDefault();
    if (e.key === "ArrowLeft" && navState.stack.length > 0 && !isDesktopTimeChooser()) {
      stepBackNav();
    } else {
